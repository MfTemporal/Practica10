...
int main() {
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		modelTemp2 = modelTemp = model = glm::translate(model, glm::vec3(0.25f, 0.0f, 0.0f)); //se quedo en la muñeca
		...
		modelTemp = model = glm::translate(model, glm::vec3(0.5f, 0.0f, 0.0f)); //se habia quedado al inicio de la muñeca
		...
		modelTemp3 = glm::translate(model, glm::vec3(0.75f, 0.0f, 0.0f));
		model = glm::rotate(model, glm::radians(dedo1), glm::vec3(0.0f, 0.0, 1.0f));
		modelTemp = model = glm::translate(model, glm::vec3(0.75f, 0.0f, 0.0f));
		model = glm::scale(model, glm::vec3(1.5f, 0.3f, 0.25f));
		color = glm::vec3(0.0f, 1.0f, 1.0f);
		...
		//////Dedo 4
		model = glm::translate(modelTemp3, glm::vec3(-0.75f, -0.65f, 0.75f));
		//model = glm::rotate(model, glm::radians(90.0f), glm::vec3(1.0f, 0.0, 0.0f));
		model = glm::rotate(model, glm::radians(pulgar1), glm::vec3(0.0f, -1.0, 0.0f));
		modelTemp = model = glm::translate(model, glm::vec3(0.5f, 0.0f, 0.0f)); //se habia quedado al inicio de la muñeca
		model = glm::scale(model, glm::vec3(1.0f, 0.3f, 0.25f));
		color = glm::vec3(0.0f, 1.0f, 1.0f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
 ...
 void Inputs(GLFWwindow *window) {
	 ...
	 if (glfwGetKey(window, GLFW_KEY_G) == GLFW_PRESS)
		 codo -= 0.18f;
	 if (glfwGetKey(window, GLFW_KEY_Y) == GLFW_PRESS and muneca < 10)
		 muneca += 0.18f;
	 if (glfwGetKey(window, GLFW_KEY_H) == GLFW_PRESS and muneca > -10) {
		 muneca -= 0.18f;
		 printf("%f", muneca);
	 }
	 if (glfwGetKey(window, GLFW_KEY_U) == GLFW_PRESS and dedo1 < 7)
		 dedo1 += 0.18f;
	 if (glfwGetKey(window, GLFW_KEY_J) == GLFW_PRESS and dedo1 > -60)
		 dedo1 -= 0.18f;
	 if (glfwGetKey(window, GLFW_KEY_I) == GLFW_PRESS and dedo2 < 1)
		 dedo2 += 0.18f;
	 if (glfwGetKey(window, GLFW_KEY_K) == GLFW_PRESS and dedo2 > -65)
		 dedo2 -= 0.18f;
	 if (glfwGetKey(window, GLFW_KEY_P) == GLFW_PRESS and pulgar1 < 20)
		 pulgar1 += 0.18f;
	 if (glfwGetKey(window, GLFW_KEY_O) == GLFW_PRESS and pulgar1 > -45)
		 pulgar1 -= 0.18f;
 }
